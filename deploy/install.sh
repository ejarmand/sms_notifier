...
install -d -m 0755 "$INSTALL_DIR" "$INSTALL_DIR/server" \
    "$INSTALL_DIR/server/blueprints" "$INSTALL_DIR/server/src" \
...
install -m 0644 "$SOURCE_DIR"/server/blueprints/*.py \
    "$INSTALL_DIR/server/blueprints/"
...
uv sync --frozen --no-dev --package sms-notifier-server --python 3.12
...
if [ ! -e "$CONFIG_DIR/authorized_keys" ]; then
    install -m 0644 /dev/null "$CONFIG_DIR/authorized_keys"
fi
